...
func upload_saves():
	...
	for save_index in range(1, num_saves + 1):
		...
		if File.new().file_exists(dat_file) and File.new().file_exists(cfg_file):
			...
			var save_cfg = yield(Firebase.Storage.ref("users/" + Firebase.Auth.auth["localid"] + "/games/evie/saves/" + save_folder + "/save.cfg").put_file(cfg_file), "task_finished")
			if File.new().file_exists(ach_file):
				var save_ach = yield(Firebase.Storage.ref("users/" + Firebase.Auth.auth["localid"] + "/games/evie/saves/" + save_folder + "/achievements.json").put_file(ach_file), "task_finished")
		else:
